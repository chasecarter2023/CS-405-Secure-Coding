// EncryptionCoding.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
//--------------------------------------------------------------------------------
// Read the entire contents of 'filename' (e.g. "inputdatafile.txt") into a std::string.
// This opens in binary mode so that no newline translations occur.
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
// Given the full file-contents string, extract the student name from line 1.
// We assume the first newline separates the name from the rest.
// Example: if the file begins with:
//    Sam Student\n
//    (other text…)
// then we return "Sam Student".
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
// Given 'source' (plaintext or ciphertext) and a non-empty 'key', produce an XOR-ciphered string.
// Because XOR is its own inverse, calling this on ciphertext with the same key returns the original plaintext.
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
// Save a new file called 'filename', with this exact format:
//   Line 1: student_name
//   Line 2: YYYY-MM-DD  (current date)
//   Line 3: key (the literal key string)
//   Line 4+: raw 'data' (which may be binary or text).
//
// We open in binary mode so that whatever bytes appear in 'data' get written verbatim.
//--------------------------------------------------------------------------------
